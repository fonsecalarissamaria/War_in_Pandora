...
typedef Item (*Apply)(Item item, void *extra);
typedef bool (*Check)(Item item, void *extra);
typedef void (*ApplyClosure)(Item item, Clausura c);

/** 
 * Aplica a funcao f a cada item de list
 * A listaA é um extra para auxiliar 
 */
void map(Lista *list, Apply f, Lista *listaA);
//Chama a função f e passa o apontador *extra para o que eu quero apontar, se não quiser passar nada  coloca NULL 
...
void concatLst(Lista lst1, Lista lst2);
